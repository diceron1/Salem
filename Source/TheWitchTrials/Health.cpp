...
void UHealth::BeginPlay()
{
	...
	TheWitchTrialsGameMode = Cast<ATheWitchTrialsGameMode>(UGameplayStatics::GetGameMode(this));
...
void UHealth::DamageTaken(AActor *DamagedActor, float Damage, const UDamageType *DamageType, AController *Instigator, AActor *DamageCauser)
{
	...
	if (Health <= 0.f && TheWitchTrialsGameMode)
	{
		TheWitchTrialsGameMode->ActorDied(DamagedActor);
	}
}
